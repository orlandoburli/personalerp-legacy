...
public abstract class BaseCadastroDAO<E extends IValueObject> extends BaseDAO {
	...
	public E get(Object[] primaryKeys) throws SQLException {
    	...
    	try {
			...
			if (list.size() == 1) { // Deve retornar somente um registro. Se retornar mais de um, retorna nulo.
				return list.get(0);
			} else {
				return null;
			}
	...
	public E get(E filter) throws SQLException {
    	...
    	try {
			E newfilter = (E) getVOClass().newInstance(); // Faz uma "copia", para setar somente os atributos chave
    ...
    public List<E> getList(E vo, int PageSize, int PageNumber) throws SQLException {
    	return getList(vo, PageSize, PageNumber, null);
    }

	@SuppressWarnings("unchecked")
	public List<E> getList(E vo, int PageSize, int PageNumber, String orderFields) throws SQLException {
		...
		try {
			...
			DaoUtils.rowSetToList(vo.getClass(), list, getRowSet(buildListStatement(vo, PageSize, PageNumber, orderFields)));
	...
	protected String buildListStatement(E vo, int pageSize, int pageNumber, String orderFields) {
...
//	}
	...
	protected String getTableName() {
		return getSchemaName() + "." + this.getVOClass().getSimpleName().replaceAll("VO", "");
	}
}