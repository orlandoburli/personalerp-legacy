...
public class InjectionFilter extends BaseFilter {
	...
	@Override
	public boolean doFilter(BaseFacade facade) throws IllegalArgumentException,
		...
		for (Field field : fields) {
			if (field.getAnnotation(InstantiateObject.class) != null) {
				try {
					...
					field.set(facade, field.getType().newInstance());
		...
		Enumeration<?> enumSession = getRequest().getSession()
				.getAttributeNames();
		while (enumSession.hasMoreElements()) {
			...
			setproperty(facade, campo, value); // Seta a propriedade
												// recursivamente
		}
		...
		while (enumParm.hasMoreElements()) {
			...
			if (value.length > 1) {
				setproperty(facade, campo, value); // Seta a propriedade como
													// array de strings
			} else if (value.length == 1) {

				Base64Decoder decoder = new Base64Decoder(value[0]);

				try {
					setproperty(facade, campo, decoder.processString());
		...
		try {
			...
			for (Field field : fields) {
				if (field.getName().equalsIgnoreCase(fieldname)) {
					...
					field.setAccessible(true);
					if (index >= 0) {
						setproperty(field.get(objeto),
								property.substring(index + 1), value);
					} else {
	...
	protected static void setField(Object container, Field field, Object value) {
		...
		try {
			if (field.getType().equals(String.class)) {
				...
				try {
					if (value != null
							&& value.toString().replace("R$", "").trim()
									.length() > 0) {
						Precision precision = field
								.getAnnotation(Precision.class);
						String mascara = "#,";
						if (precision != null) {
							for (int i = 0; i < precision.decimals(); i++) {
								mascara += "0";
							}
						} else {
							mascara += "00";
						...
						formater.setCurrency(Currency.getInstance(new Locale(
								"pt", "BR")));
						...
						if (value instanceof BigDecimal) {
							...
							if (field.getType().equals(BigDecimal.class)) {
								...
								// new ParsePosition(0)));
								BigDecimal valor = (BigDecimal) formater
										.parse(value.toString()
												.replace("R$", "").trim());
								if (precision != null) {
									valor = valor
											.setScale(precision.decimals());
								}
								field.set(container, valor);
							} else {
								Double doubleval = formater.parse(
				...
				try {
					if (value != null && value.toString().trim().length() == 10) {
						...
						Date date = new Date(sdf.parse(value.toString())
								.getTime());
				...
				if (value != null && value.toString().length() <= 5) {
					time = Timestamp.valueOf("0001-01-01 " + value.toString()
							+ ":00");
				} else if (value != null
						&& value.toString().trim().length() == 10) {
					String valueof = value.toString().substring(6, 10) + "-"
							+ value.toString().substring(3, 5) + "-"
							+ value.toString().substring(0, 2) + " 00:00:00";
				...
				field.set(container, time);
					...
					&& value.getClass().equals(String.class)) { // Para
																// arrays
	...
	public boolean doFilter(BasePage page) throws IllegalArgumentException,
		...
		for (Field field : fields) {
			if (field.getAnnotation(InstantiateObject.class) != null) {
				try {
					field.setAccessible(true);
					field.set(page, field.getType().newInstance());
				} catch (InstantiationException e) {
					// Se der excecao, é porque nao possui construtor sem
					// parametros
					e.printStackTrace();
		...
		while (enumSession.hasMoreElements()) {
			String campo = enumSession.nextElement().toString();
			Object value = getRequest().getSession().getAttribute(campo);
			setproperty(page, campo, value); // Seta a propriedade
												// recursivamente
		}
		...
		while (enumParm.hasMoreElements()) {
			...
			String[] value = getRequest().getParameterValues(campo);
			if (value.length > 1) {
				setproperty(page, campo, value); // Seta a propriedade como
													// array de strings
			} else if (value.length == 1) {
				...
				try {
					setproperty(page, campo, decoder.processString());
				} catch (Base64FormatException e) {
					e.printStackTrace();
				}
			}
